...
export default class PartidaController {
  ...
  // validações mínimas do payload pra evitar 500
  ...
  // evita duplicidade pelo "codigo" (id da GC)
  const jaExiste = await Partidas.query().where('codigo', Number(data.id)).first();
  if (jaExiste) {
    return this.customResponse.erro(
      response,
  ...
  const resolveAvatarUrl = (j: any): string => {
    try {
      ...
      const mHtml = html.match(/src="([^"]+_medium\.(?:jpg|jpeg|png|webp))"/i);
      ...
      if (avatar && typeof avatar === 'string') {
        if (/^https?:\/\//i.test(avatar)) {
          if (/_medium\.(jpg|jpeg|png|webp)(\?.*)?$/i.test(avatar)) return avatar;
          const hasExt = /\.(jpg|jpeg|png|webp)(\?.*)?$/i.test(avatar);
          if (hasExt) return avatar.replace(/\.(jpg|jpeg|png|webp)(\?.*)?$/i, `_medium.$1`);
      ...
      if (plAvatar && typeof plAvatar === 'string') {
        if (/^https?:\/\//i.test(plAvatar)) {
          if (/_medium\.(jpg|jpeg|png|webp)(\?.*)?$/i.test(plAvatar)) return plAvatar;
          const hasExt = /\.(jpg|jpeg|png|webp)(\?.*)?$/i.test(plAvatar);
          if (hasExt) return plAvatar.replace(/\.(jpg|jpeg|png|webp)(\?.*)?$/i, `_medium.$1`);
  ...
  try {
    ...
    // cria a partida — SEM spread de objetos desconhecidos
    ...
    // normaliza jogadores das duas equipes
    const jogadoresInput = [
      ...(data?.jogos?.players?.team_a || []).map((j: any) => ({
        ...
        kda_player: j?.kdr,
        kast: toNum(String(j?.pkast ?? '').toString().replace('%', '')),
    ...
    for (const jogador of jogadoresInput) {
      ...
      // busca com client na trx
      let jogadorModel =
        (await Jogadores.query({ client: trx }).where('nome', nome).first()) || null;

      // tenta vincular ao usuario_adm por nome_normalizado (se ainda não tiver)
      if (jogadorModel && !jogadorModel.usuario_adm_id) {
        const nomeNorm = normName(nome);
      ...
      pontosPartida += estatisticas.adr < 50 ? 5 : estatisticas.adr <= 100 ? 10 : 20;
      pontosPartida += vitoria ? 20 : 10;

      if (jogadorModel) {
        if ((!jogadorModel.imagem || jogadorModel.imagem.trim() === '') && imagem) {
        ...
        if (vitoria) {
          jogadorModel.vitorias = (Number(jogadorModel.vitorias || 0) + 1).toString();
        }

        jogadorModel.kills = (Number(jogadorModel.kills || 0) + estatisticas.kills).toString();
        jogadorModel.assistencias = (
          Number(jogadorModel.assistencias || 0) + estatisticas.assistencias
        ).toString();
        jogadorModel.mortes = (Number(jogadorModel.mortes || 0) + estatisticas.mortes).toString();
        ...
        jogadorModel.adr = (
          ((Number(jogadorModel.adr || 0) || 0) * (novaQtdPartidas - 1) + estatisticas.adr) /
          novaQtdPartidas
        ).toFixed(2);
        ...
        const pontosTotaisAnterior = Number(jogadorModel.pontos || 0) * (novaQtdPartidas - 1);
    ...
    const pivotData = jogadoresCriados.reduce((acc, jogador) => {
      ...
      acc[id] = {
        ...origem,
        adr: Number(origem.adr).toFixed ? Number(origem.adr).toFixed(2) : String(origem.adr),
    ...
    // === crédito de gold (com proteção a duplicidade) ===
    ...
    return this.customResponse.erro(response, 'Erro ao importar partida!', error, 500);
  }
}
